...
final videoZoomControllerProvider = StateProvider.autoDispose<TransformationController>((ref) {
...
class CustomVideoPlayerWidget extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CustomVideoPlayerWidget> createState() => _CustomVideoPlayerWidgetState();
}

class _CustomVideoPlayerWidgetState extends ConsumerState<CustomVideoPlayerWidget> with WidgetsBindingObserver {
  ...
  void initState() {
    super.initState();
    // Tambahkan observer untuk app lifecycle state
    WidgetsBinding.instance.addObserver(this);
    
    // Inisialisasi video player menggunakan provider state
  ...
  void dispose() {
    ...
    // Pastikan kita tidak mendispose controller di sini karena dikelola oleh provider
    super.dispose();
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.paused) {
      // App masuk ke background, pause video
      _pauseVideo();
    }
  }
  ...
  void _pauseVideo() {
    if (_videoController != null && _videoController!.value.isPlaying) {
      ...
      // Juga update state provider jika menggunakan controller dari provider
      ref.read(videoPlayerStateProvider.notifier).pauseVideo();
      if (mounted) setState(() {});
    }
  }
  ...
  Future<void> _initializeFromProvider() async {
    ...
    try {
      // Gunakan provider state jika tersedia
      final videoState = ref.read(videoPlayerStateProvider);
      
      // Jika provider belum punya controller yang sesuai atau controller tidak match dengan mediaItem ini,
      // minta provider untuk load video
      if (widget.mediaItem.type == 'video' && !_isDisposed) {
        // Force reload video untuk memastikan video yang sesuai selalu dimuat
        await ref.read(videoPlayerStateProvider.notifier).loadVideo(widget.mediaItem);
        
        // Tunggu sebentar untuk memastikan controller diinisialisasi
        await Future.delayed(const Duration(milliseconds: 200));
        
        // Setelah provider load video, ambil controller-nya
        final updatedState = ref.read(videoPlayerStateProvider);
        if (!_isDisposed && updatedState.controller != null) {
          _videoController = updatedState.controller;
          
          // Pastikan video dimulai dengan memanggil play
          ...
          if (!_isDisposed && mounted) {
            final retryState = ref.read(videoPlayerStateProvider);
            if (retryState.controller != null) {
              _videoController = retryState.controller;
              if (_videoController!.value.isInitialized && !_videoController!.value.isPlaying) {
                _videoController!.play();
              }
              setState(() {
              ...
              setState(() {
                _isLoading = false;
                _errorMessage = 'Failed to load video controller';
              });
            }
          }
        }
  ...
  void _toggleFullControls() {
    ...
    ref.read(videoFullControlsProvider.notifier).state = !ref.read(videoFullControlsProvider);
  ...
  Widget build(BuildContext context) {
    // Listen to videoPlayerStateProvider untuk update
    ...
    // Sinkronkan state lokal dengan provider state
    if (videoState.controller != null && videoState.controller != _videoController) {
      _videoController = videoState.controller;
      _isInitialized = _videoController!.value.isInitialized;
      _isLoading = videoState.isLoading;
    }
    
    if (videoState.isLoading || !_isInitialized) {
      return _buildLoadingWidget();
    }

    if (videoState.errorMessage != null || _errorMessage != null) {
      return Center(
        child: Text(
          videoState.errorMessage ?? _errorMessage ?? "Error loading video",
    ...
    return GestureDetector(
      ...
      child: Stack(
        ...
        children: [
          ...
          if (showControls && !_isZoomed && _videoController != null)
            Positioned.fill(
